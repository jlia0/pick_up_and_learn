...
class TCardState extends State<TCard> with TickerProviderStateMixin {
  ...
  void _judgeRunAnimation(DragEndDetails details, Size size) {
    ...
    final bool isSwipeTopLeft = _frontCardAlignment.x < -limit && _frontCardAlignment.y < -limit;
    final bool isSwipeTopRight = _frontCardAlignment.x > limit && _frontCardAlignment.y < -limit;
    final bool isSwipeBotLeft = _frontCardAlignment.x < -limit && _frontCardAlignment.y > limit;
    final bool isSwipeBotRight = _frontCardAlignment.x > limit && _frontCardAlignment.y > limit;

    // 判断是否运行向前的动画，否则回弹
    if (isSwipeTopLeft || isSwipeTopRight || isSwipeBotLeft || isSwipeBotRight) {
      _runChangeOrderAnimation();

      if (isSwipeTopLeft) {
        _swipeInfoList.add(SwipeInfo(_frontCardIndex, SwipeDirection.TopLeft));
      } else if (isSwipeTopRight){
        _swipeInfoList.add(SwipeInfo(_frontCardIndex, SwipeDirection.TopRight));
      }else if (isSwipeBotLeft){
        _swipeInfoList.add(SwipeInfo(_frontCardIndex, SwipeDirection.BotLeft));
      }else{
        _swipeInfoList.add(SwipeInfo(_frontCardIndex, SwipeDirection.BotRight));
      }
    } else {
